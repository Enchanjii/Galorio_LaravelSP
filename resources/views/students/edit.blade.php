    ...
    <div class="container py-5">
        ...
        <form method="POST" action="{{ url('/students/' . $student['id']) }}">
            ...
            <div class="mb-3">
                ...
                <select name="course" id="courseSelect" class="form-control" required style="display: none;">
                    ...
                    @foreach ($courses as $code => $fullName)
                        <option value="{{ $code }}" data-fullname="{{ $fullName }}" @if (old('course', $student['course']) === $code) selected @endif>
                            {{ $fullName }}
                        </option>
                    @endforeach
                </select>
            ...
            <script>
                ...
                // Initialize with current selected course name
            ...
            <div class="mb-3">
                ...
                <select name="year" class="form-control" required>
                    ...
                    <option value="1" @if (old('year', $student['year']) === '1') selected @endif>1st Year</option>
                    <option value="2" @if (old('year', $student['year']) === '2') selected @endif>2nd Year</option>
                    <option value="3" @if (old('year', $student['year']) === '3') selected @endif>3rd Year</option>
                    <option value="4" @if (old('year', $student['year']) === '4') selected @endif>4th Year</option>
                    <option value="5" @if (old('year', $student['year']) === '5') selected @endif>5th Year</option>
                </select>
                @error('year')
                    <div class="text-danger small">{{ $message }}</div>
                @enderror
